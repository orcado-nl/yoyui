...
function handleSonarNested2(parseText, param) {
    return {
        name: param.name, // optional: param.flags.isOptional,
        type: param.type.toString(),
        description: param.comment?.summary.map((s) => parseText(s.text || '')).join(' ')
    };
}
...
const library = { name: 'PrimeReact', version: pkg.version, repository: pkg.repository, license: pkg.license };
const webTypes = {
    ...
    contributions: { html: { 'types-syntax': 'typescript', 'description-markup': 'markdown', tags: [], attributes: [] } }
};
app.bootstrap({
    ...
    externalSymbolLinkMappings: {
        '@types/react': { 'React.ReactNode': 'https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/react/index.d.ts', 'React.CSSProperties': 'https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/react/index.d.ts' }
    }
});
...
if (project) {
    ...
    project.children.forEach((module) => {
        ...
        if (description) {
            ...
            const urlMatch = urlRegex.exec(description);
            if (urlMatch?.[2]) {
                url = urlMatch[2];
            }
        }
        const tag = { name: capitalizedName, source: { module: library.name, symbol: capitalizedName }, description, 'doc-url': url };
            ...
            component_method_group?.children.forEach((method) => {
                ...
                methods.values.push({
                    ...
                    parameters: signature.parameters.map((param) => {
                        return { name: param.name, type: param.type.toString(), description: param.comment?.summary.map(handleSonarNested1Callback.bind(null)).join(' ') };
                    }),
                    returnType: signature.type.toString(),
                    description: signature.comment && signature.comment.summary.map(handleSonarNested1.bind(null)).join(' ')
                });
            });
            const component_props_id = component.extendedTypes?.[0].typeArguments?.[0]?._target;
            const module_properties_group = module.groups.find((g) => g.title === 'Properties');
            const component_props = module_properties_group?.children.find((c) => (component_props_id ? c.id === component_props_id : true));
            const props = [];
            const callbacks = [];
            if (component_props) {
                ...
                component_props_group?.children.forEach((prop) => {
                    if (!prop.inheritedFrom || (prop.inheritedFrom && !prop.inheritedFrom.toString().startsWith('Omit.data-pr-'))) {
                        props.push({
                            name: prop.name,
                            default: prop.comment?.getTag('@defaultValue') ? parseText(prop.comment.getTag('@defaultValue').content[0].text) : 'null',
                            description: prop.comment?.summary.map((s) => parseText(s.text || '')).join(' '),
                            value: { kind: 'expression', type: prop.type.toString() } //optional: prop.flags.isOptional,
                ...
                component_props_methods_group?.children.forEach((method) => {
                    const signature = method.getAllSignatures()[0];
                    callbacks.push({ name: signature.name, description: signature.comment.summary.map((s) => parseText(s.text || '')).join(' '), arguments: signature.parameters.map(handleSonarNested2.bind(null, parseText)) });
                });
            }
        ...
        module_model_group?.children.forEach((model) => {
            ...
            model_props_group?.children.forEach((prop) => {
                props.values.push({
                    ...
                    default: prop.comment?.getTag('@defaultValue') ? prop.comment.getTag('@defaultValue').content[0].text : '',
                    description: prop.comment && prop.comment.summary.map((s) => s.text || '').join(' ')
                });
            });
            doc[name]['model'][model.name] = { description: event_props_description, props };
        });
        ...
        module_functions_group?.children.forEach((method) => {
            ...
            if (signatures && signatures.length > 0) {
                ...
                doc[name]['functions'].values[method.name] = {
                    name: signature.name,
                    parameters: signature.parameters.map((param) => {
                        return { name: param.name, type: param.type.toString(), description: param.comment?.summary.map((s) => s.text || '').join(' ') };
                    }),
                    returnType: signature.type.toString(),
            ...
            if (event.groups) {
                ...
                event_methods_group?.children.forEach((method) => {
                    ...
                    callbacks.push({
                        name: signature.name,
                        parameters: signature.parameters.map((param) => {
                            return { name: param.name, optional: param.flags.isOptional, type: param.type.toString(), description: param.comment?.summary.map(handleSonarNested2Callback.bind(null, parseText)).join(' ') };
                        }),
                        returnType: signature.type.toString(),
        ...
        module_types_group?.children.forEach((event) => {
            ...
            doc[name]['types'].values[event.name] = { values, description: event_props_description };
        });
    });
