...
export const TabView = React.forwardRef((inProps, ref) => {
    ...
    const metaData = { props, state: { id: idState, isPrevButtonDisabled: backwardIsDisabledState, isNextButtonDisabled: forwardIsDisabledState, hiddenTabsState: hiddenTabsState, activeIndex: activeIndexState } };
    const { ptm, ptmo, cx, sx, isUnstyled } = TabViewBase.setMetaData({ ...metaData });

    useHandleStyle(TabViewBase.css.styles, isUnstyled, { name: 'tabview' });

    const getTabPT = (tab, key, index) => {
        const tabMetaData = { props: tab.props, parent: metaData, context: { index, count, first: index === 0, last: index === count - 1, active: index == activeIndexState, disabled: getTabProp(tab, 'disabled') } };
    ...
    const onTabHeaderClose = (event, index) => {
        ...
        const { key } = children[index]; // give caller a chance to stop the selection
    ...
    const changeActiveIndex = (event, tab, index) => {
        ...
        if (!getTabProp(tab, 'disabled')) {
            ...
            if (props.onBeforeTabChange?.({ originalEvent: event, index }) === false) {
                return;
            }
    ...
    const updateButtonState = () => {
        ...
        setForwardIsDisabledState(Number.parseInt(scrollLeft) === scrollWidth - width);
    };
    ...
    const navBackward = () => {
        ...
        contentRef.current.scrollLeft = Math.max(0, pos);
    };
    ...
    const navForward = () => {
        ...
        contentRef.current.scrollLeft = Math.min(lastPos, pos);
    };
    ...
    useUpdateEffect(() => {
        if (ObjectUtils.isNotEmpty(hiddenTabsState)) {
            const tabInfo = findVisibleActiveTab(hiddenTabsState.at(-1));
    ...
    React.useImperativeHandle(ref, () => ({ props, reset, getElement: () => elementRef.current }));
    ...
    const createTabHeader = (tab, index) => {
        ...
        const headerTitleProps = mergeProps({ className: cx('tab.headertitle') }, getTabPT(tab, 'headertitle', index));
        ...
        const closeIconProps = mergeProps(
            { className: cx('tab.closeIcon'), onClick: (e) => onTabHeaderClose(e, index), onKeyDown: (e) => onCloseIconKeyDown(e, index), tabIndex: 0, 'aria-label': ariaLabel('close') || 'Close' },
        ...
        const headerProps = mergeProps(
            { ref: (el) => (tabsRef.current[`tab_${index}`] = el), className: cx('tab.header', { selected, disabled, headerClassName, _className }), style: sx('tab.header', { headerStyle, _style }), role: 'presentation' },
    ...
    const createNavigator = () => {
        ...
        const navContentProps = mergeProps({ id: idState + '_navcontent', ref: contentRef, className: cx('navcontent'), style: props.style, onScroll }, ptm('navcontent'));
        const navProps = mergeProps({ ref: navRef, className: cx('nav'), role: 'tablist' }, ptm('nav'));
        const inkbarProps = mergeProps({ ref: inkbarRef, 'aria-hidden': 'true', role: 'presentation', className: cx('inkbar') }, ptm('inkbar'));
    ...
    const createContent = () => {
        const panelContainerProps = mergeProps({ className: cx('panelcontainer'), style: props.panelContainerStyle }, ptm('panelcontainer'));
    ...
    const createPrevButton = () => {
        const prevIconProps = mergeProps({ 'aria-hidden': 'true' }, ptm('previcon'));
        const icon = props.prevButton || <ChevronLeftIcon {...prevIconProps} />;
        const leftIcon = IconUtils.getJSXIcon(icon, { ...prevIconProps }, { props });
        const prevButtonProps = mergeProps({ ref: prevBtnRef, type: 'button', className: cx('prevbutton'), 'aria-label': ariaLabel('prevPageLabel'), onClick: (e) => navBackward() }, ptm('prevbutton'));
    ...
    const createNextButton = () => {
        const nextIconProps = mergeProps({ 'aria-hidden': 'true' }, ptm('nexticon'));
        const icon = props.nextButton || <ChevronRightIcon {...nextIconProps} />;
        const rightIcon = IconUtils.getJSXIcon(icon, { ...nextIconProps }, { props });
        const nextButtonProps = mergeProps({ ref: nextBtnRef, type: 'button', className: cx('nextbutton'), 'aria-label': ariaLabel('nextPageLabel'), onClick: (e) => navForward() }, ptm('nextbutton'));
    ...
    const rootProps = mergeProps({ id: idState, ref: elementRef, style: props.style, className: classNames(props.className, cx('root')) }, TabViewBase.getOtherProps(props), ptm('root'));
    const navContainerProps = mergeProps({ className: cx('navcontainer') }, ptm('navcontainer'));
