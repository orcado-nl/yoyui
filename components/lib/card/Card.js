...
export const Card = React.forwardRef((inProps, ref) => {
    ...
    const { ptm, cx, isUnstyled } = CardBase.setMetaData({ props });
    ...
    const createBody = () => {
        const titleProps = mergeProps({ className: cx('title') }, ptm('title'));
        const title = props.title && <div {...titleProps}>{ObjectUtils.getJSXElement(props.title, props)}</div>;
        const subTitleProps = mergeProps({ className: cx('subTitle') }, ptm('subTitle'));
    ...
    const rootProps = mergeProps({ id: props.id, ref: elementRef, style: props.style, className: classNames(props.className, cx('root')) }, CardBase.getOtherProps(props), ptm('root'));
