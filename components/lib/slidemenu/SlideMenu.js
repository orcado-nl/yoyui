...
export const SlideMenu = React.memo(
    React.forwardRef((inProps, ref) => {
        ...
        const { ptm, cx, sx, isUnstyled } = SlideMenuBase.setMetaData({ props, state: { id: idState, visible: visibleState, level: levelState } });
        ...
        React.useImperativeHandle(ref, () => ({ props, toggle, show, hide, navigateForward, navigateBack, setLevelState, getElement: () => menuRef.current }));

        const createBackward = () => {
            const previousIconProps = mergeProps({ className: cx('previousIcon') }, ptm('previousIcon'));
            ...
            const previousProps = mergeProps({ ref: backward, className: cx('previous', { levelState }), onClick: (e) => navigateBack() }, ptm('previous'));
        ...
        const createElement = () => {
            ...
            const rootProps = mergeProps({ ref: menuRef, id: props.id, className: classNames(props.className, cx('root')), style: props.style, onClick: (e) => onPanelClick(e) }, SlideMenuBase.getOtherProps(props), ptm('root'));
            const wrapperProps = mergeProps({ className: cx('wrapper'), style: wrapperStyle }, ptm('wrapper'));
            const contentProps = mergeProps({ ref: slideMenuContent, className: cx('content') }, ptm('content'));
            const transitionProps = mergeProps(
                { classNames: cx('transition'), in: !props.popup || visibleState, timeout: { enter: 120, exit: 100 }, options: props.transitionOptions, unmountOnExit: true, onEnter, onEntered, onExit, onExited },
                ptm('transition')
            );
