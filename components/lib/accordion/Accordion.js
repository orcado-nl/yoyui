...
export const Accordion = React.forwardRef((inProps, ref) => {
    ...
    const metaData = { props, state: { id: idState, activeIndex: activeIndexState } };
    const { ptm, ptmo, cx, isUnstyled } = AccordionBase.setMetaData({ ...metaData });
    ...
    const getTabPT = (tab, key, index) => {
        const tabMetaData = { parent: metaData, context: { index, count, first: index === 0, last: index === count - 1, selected: isSelected(index), disabled: getTabProp(tab, 'disabled') } };
    ...
    const changeActiveIndex = (event, tab, index) => {
        if (!getTabProp(tab, 'disabled')) {
            ...
            if (props.onTabChange) {
                props.onTabChange({ originalEvent: event, index: newActiveIndex });
            } else {
    ...
    const isSelected = (index) => {
        return props.multiple && Array.isArray(activeIndex) ? activeIndex.includes(index) : activeIndex === index;
    };
    ...
    const createTabHeader = (tab, selected, index) => {
        ...
        const headerTitleProps = mergeProps({ className: cx('accordiontab.headertitle') }, getTabPT(tab, 'headertitle', index));
        ...
        const headerIconProps = mergeProps({ 'aria-hidden': 'true', className: cx('accordiontab.headericon') }, getTabPT(tab, 'headericon', index));
        ...
        const headerProps = mergeProps(
            { className: classNames(getTabProp(tab, 'headerClassName'), getTabProp(tab, 'className'), cx('accordiontab.header', { selected, getTabProp, tab })), style, 'data-p-highlight': selected, 'data-p-disabled': getTabProp(tab, 'disabled') },
    ...
    const createTabContent = (tab, selected, index) => {
        ...
        const toggleableContentProps = mergeProps(
            { id: contentId, ref: contentRef, className: classNames(getTabProp(tab, 'contentClassName'), getTabProp(tab, 'className'), cx('accordiontab.toggleablecontent')), style, role: 'region', 'aria-labelledby': ariaLabelledby },
            getTabPT(tab, 'toggleablecontent', index)
        );
        const contentProps = mergeProps({ className: cx('accordiontab.content') }, getTabPT(tab, 'content', index));
        const transitionProps = mergeProps({ classNames: cx('accordiontab.transition'), timeout: { enter: 1000, exit: 450 }, in: selected, unmountOnExit: true, options: props.transitionOptions }, getTabPT(tab, 'transition', index));
    ...
    const createTab = (tab, index) => {
        if (ObjectUtils.isValidChild(tab, 'AccordionTab')) {
            ...
            const rootProps = mergeProps({ className: cx('accordiontab.root', { selected }) }, AccordionTabBase.getCOtherProps(tab), getTabPT(tab, 'root', index));
    ...
    const rootProps = mergeProps({ className: classNames(props.className, cx('root')), style: props.style }, AccordionBase.getOtherProps(props), ptm('root'));
