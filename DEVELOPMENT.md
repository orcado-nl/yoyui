...
This will alter the bundler to only emit non-minified esm modules. The aliasing plugin has also been disabled for components. Once everything has been bundled (this can take a few minutes) you should keep this command running. It will allow for incremental builds as you develop in the `yoyui/` directory.
...
### Open a new terminal and navigate to the `yoyui/dist/` directory

Run yalc publish

```shell
~/yoyui/ $ cd dist
~/yoyui/dist/ $ npx yalc publish
```

### Now change your directory to your local project you are developing on!

```shell
~/yoyui/dist/ $ cd ~/my-cool-project
~/my-cool-project/ $
```

The goal now is to link your @orcado/yoyui dependency to the yalc package that we configured earlier:

```shell
~/my-cool-project/ $ npx yalc add @orcado/yoyui
```
...
Yalc dependencies found: [ '@orcado/yoyui' ]
```

After doing your changes in the `yoyui/` directory, you can publish the changes to your local project by running:

```shell
~/yoyui/dist/ $ npx yalc push
```
...
```shell
~/yoyui $ npm run dev:link -- -- --watch.onEnd="cd dist && npx yalc push"
```
...
```shell
~/my-cool-project/ $ yalc remove @orcado/yoyui
~/my-cool-project/ $ rm -rf yalc.lock .yalc
~/my-cool-project/ $ npm install @orcado/yoyui@latest
```

> Note: `yalc` stores the published package in `~/.yalc/@orcado/yoyui` and uses file references in your project.
